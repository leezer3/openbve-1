				...
				if (Direction < 0) {
					...
					if (TriggerType == EventTriggerType.FrontCarFrontAxle) {
						...
						if (Train.Specs.CurrentAverageSpeed > this.NextSpeedLimit) {
							Game.AddMessage(Strings.GetInterfaceString("message_route_overspeed"), Game.MessageDependency.RouteLimit, Interface.GameMode.Normal, Game.MessageColor.Orange, double.PositiveInfinity);
						}
					} else if (TriggerType == EventTriggerType.RearCarRearAxle) {
						int n = Train.RouteLimits.Length;
						if (n > 0) {
